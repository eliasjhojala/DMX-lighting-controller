...
class fixureInput {
  ...
  int fT = fixtures[sF].fixtureType;
  ...
  void receiveOSC(int value1, int value2, int address) {
            ...
            //CH 6: Colour wheel
            colorChange(address); //Check color buttons every round
            if(address.equals("/8/rainbow" + str(1))) { rainbow(value); } //Color rainbow
            
            //CH 7: Shutter
            if(address.equals("/8/blackOut" + str(1)) && value == 1) { blackOut(id); } //Check if blackout is presset
            if(address.equals("/8/openShutter" + str(1)) && value == 1) { openShutter(id); } //Check if open is pressed
            ...
            //CH 9: Gobo wheel
            if(address.equals("/8/noGobo" + str(1)) && value == 1) { noGobo(id); } //Gobowheel open
            if(address.equals("/8/goboUp" + str(1)) && value == 1) { goboUp(id); } //Next gobo
            if(address.equals("/8/goboDown" + str(1)) && value == 1) { goboDown(id); } //Reverse gobo
            ...
            if(address.equals("/8/goboNoRotation" + str(1)) && value == 1) { goboNoRotation(id); } //No gobo rotation
            if(address.equals("/8/goboBouncing" + str(1)) && value == 1) { goboBouncing(id); } //Gobo bouncing
            ...
            if(address.equals("/8/autoProgram" + str(1)) && value == 1) { autoProgram(id); } //Next autoProgram
    ...
    void autoProgram() {
      if(ftIsMhX50) {
        ...
        autoProgram = mhx50_autoProgram_values[mhx50_autoProgramNumber[id]];
      }
    }
      ...
      for(int i = 0; i < mhx50_color_names.length; i++) {
        if(address != null) {
          if(address.equals("/8/" + mhx50_color_names[i] + str(id + 1))) {
            mhx50_colorNumber = i; break; //This is to show right rgb values in visualisation
          }
        }
      }
      red = mhx50_colorValues[mhx50_colorNumber][0];
      green = mhx50_colorValues[mhx50_colorNumber][1];
      blue = mhx50_colorValues[mhx50_colorNumber][2];
      
      if(ftIsMhX50) { colorWheel = mhx50_color_values[mhx50_colorNumber]; } //Gives right value to moving head color channel
    }
