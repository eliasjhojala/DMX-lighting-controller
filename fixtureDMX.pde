  ...
  */
  ...
  void presetProcess() {
    for(int i = 0; i < DMXlength; i++) {
      if(valueHasChanged(i, DMX, DMXold)) {
  ...
  boolean receiveDMX(int[] dmxChannels) {
    try {
      switch(parent.fixtureTypeId) {
           ...
           /* simple rgb led par with dim */   case 19: dimmer = dmxChannels[3]; red = dmxChannels[0]; green = dmxChannels[1]; blue = dmxChannels[2]; break; //Simple rgb led par with dim
  ...
  int[] getDMX() {
   int[] dmxChannels = new int[30];
      switch(parent.fixtureTypeId) {
       ...
       /* simple rgb led par with dim */   case 19: dmxChannels = new int[4]; dmxChannels[3] = dimmer; dmxChannels[0] = red; dmxChannels[1] = green; dmxChannels[2] = blue; break; //Simple rgb led par with dim
