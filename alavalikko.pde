...
void alavalikko() {
  ...
  for(int i = 0; i <= channels/2; i++) {
    translate(65, 0);
    fixture(i, dimInput[(-1)+fixtureChannelNow[fixtureIdNow[i]]+1], fixtureChannelNow[fixtureIdOriginal[fixtureIdNow[i]]], (-1)+fixtureChannelNow[fixtureIdNow[i]]+1, fixtureType1[fixtureIdNow[i]]);
  }
  for(int i = 0; i <= channels/2; i++) {
    translate(-65, 0);
  }
...
void fixture(int numero3, int dimmi, int numero2, int numero, int fixtuuriTyyppi1) {
      ...
      if(mouseY > height-(100+15) && mouseY < height-100) {
        if(mouseClicked == true) {
          if(numero3-1 >= 0 && numero3-1 <= fixtureIdNow.length) {
            dimInput[(-1)+fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 255; 
          }      
        }
        else if(mouseReleased == true){
          mouseReleased = false;
          dimInput[(-1)+fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 0;
        }
      }
    }
  }
    ...
    if((mouseX > 65*(numero3-(channels/2))+10) && (mouseX < 65*(numero3+1-(channels/2)))) {
      ...
      if(mouseY > height-(100+15-65) && mouseY < height-110) {
          ...
          if(mouseY > height-(100+15-65) && mouseY < height-(100-65)) {
            if(mouseClicked == true) {
                dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 255;
            }
            else if(mouseReleased == true){
              mouseReleased = false;
              dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 0;
            }
          }
        }
      }
  ...
  if(mouseX > 65*numero3+10 && mouseX < 65*(numero3+1)) {
      if(mouseY > height-(100+30) && mouseY < height-(100+15)) {
        if(mouseClicked == true) {
          ...
          if(dimInput[(-1)+fixtureChannelNow[fixtureIdNow[numero3-1]]+1] == 255) {
                dimInput[(-1)+fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 0;
                print(numero2);
                print(":");
                println(0);
    ...
    if((mouseX > 65*(numero3-(channels/2))+10) && (mouseX < 65*(numero3+1-(channels/2)))) {
          if(mouseY > height-(100+30-65) && mouseY < height-(100-65+15)) {
            if(mouseClicked == true) {
              if(mouseReleased == true) {
              if(dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] == 255) {
                dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 0;
              }
              else {
                dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = 255;
  ...
  if(mouseX > 65*numero3 && mouseX < 65*(numero3)+10) {
      if(mouseY > height-(100+30) && mouseY < height-100) {
        if(mouseClicked == true) {
          dimInput[(-1)+fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = round(map(mouseY, height-100, height-(100+30), 0, 255));
    ...
    if((mouseX > 65*(numero3-(channels/2))) && (mouseX < 65*(numero3-(channels/2))+10)) {
          if(mouseY > height-(100+30-65) && mouseY < height-(100-65)) {
            if(mouseClicked == true) {
              dimInput[fixtureChannelNow[fixtureIdNow[numero3-1]]+1] = round(map(mouseY, height-(100-65), height-(100+30-65), 0, 255));
            }
            else if(mouseReleased == true) {
              mouseReleased = false;
            }
